...
if ( "${GNUSTEP_MAKE_STRICT_V2_MODE}" == "yes" ) then
  ...
  if ( ! ${?GNUSTEP_SYSTEM_ROOT} ) then
    setenv GNUSTEP_SYSTEM_ROOT "/usr/System"
  endif

  if ( ! ${?GNUSTEP_LOCAL_ROOT} ) then
    setenv GNUSTEP_LOCAL_ROOT "/usr/Local"
  endif

  if ( ! ${?GNUSTEP_NETWORK_ROOT} ) then
    setenv GNUSTEP_NETWORK_ROOT "/usr/Network"
  endif
...
if ( ! ${?GNUSTEP_MAKEFILES} ) then
  setenv GNUSTEP_MAKEFILES "/usr/share/GNUstep/Makefiles"
endif
...
if ( "" == "" ) then
  setenv GNUSTEP_HOST "x86_64-mandriva-linux-gnu"
  setenv GNUSTEP_HOST_CPU "x86_64"
  setenv GNUSTEP_HOST_VENDOR "mandriva"
